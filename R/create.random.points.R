...
# trainingareas:  SpatialpointsDataframe of the training areas in which the random points have to be chosen.
#                 One of the columns of the dataframe should repressent the different crop types.
# raster:         raster on which the points should be created
...
create.random.points <- function(trainingareas,raster,crop_types,crop_column_no,crop_numbers,samplesize){
  emptylist<-list()
  for (i in 1:length(crop_types)){
    ...
    cropPoints <-try(SpatialPoints(randomPoints(mask(raster,subset(trainingareas,get(names(trainingareas)[crop_column_no])==crop)),samplesize),CRS(projection(trainingareas))))
    cropPoints <- try(SpatialPointsDataFrame(cropPoints,data=data.frame(rep(crop,length(cropPoints)),rep(cropNO,length(cropPoints)))))
    names(cropPoints)<-c('Crop','CropNO')
    emptylist[i]<-cropPoints
    # if there are no points created for a certain crop type: return error & break
    if (class(cropPoints)=="try-error"){
      print(paste('No random points created for ',crop,'. There were no trainingareas without bare soil found for this crop. Random point creation aborted.',sep=''))
      break
